...
const NO_LINES_TEST_FILEPATHS = [
	"testdata/empty-file-test1.csv",
	"testdata/empty-file-test2.csv"
];
	...
	for (let fileName of fileNames) {
		if (path.extname(fileName) == ".csv") {
			...
			try {
				...
				if (error.name == "NoLines" && NO_LINES_TEST_FILEPATHS.includes(testDataPath)) {
					console.log(`No lines test passed: ${testDataPath}`);
				} else {
					throw error;
				}
			}
